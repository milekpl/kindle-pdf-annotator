...
def test_basic_normalization():
    ...
    if normalized_pdf == normalized_clipping:
        print("✓ PERFECT MATCH after normalization!")
    else:
        print("✗ Still different after normalization")
        # Find differences
        for i, (c1, c2) in enumerate(zip(normalized_pdf, normalized_clipping)):
            if c1 != c2:
                print(f"  First diff at position {i}: PDF='{c1}' vs Clipping='{c2}'")
                print(f"  PDF context: ...{normalized_pdf[max(0,i-20):i+20]}...")
                print(f"  Clipping context: ...{normalized_clipping[max(0,i-20):i+20]}...")
                break
...
def test_clipping_with_errors():
    ...
    print(f"  - 'content-specifc' (missing 'i')")
    print(f"  - 'specialpurpose' (no space/hyphen)")
    print()
    
    # Try to find the clipping in the PDF
    if normalized_clipping_err in normalized_pdf:
        print("✓ EXACT MATCH despite Kindle errors!")
    else:
        print("✗ No exact match due to Kindle extraction errors")
        print("\nSearching for what Kindle extracted vs what PDF has:")
        
        # Find specific error patterns
        if 'content-specifc' in normalized_clipping_err:
            print("  Kindle has: 'content-specifc' (missing 'i')")
            if 'content-specific' in normalized_pdf:
                print("  PDF has: 'content-specific' (correct)")
        
        if 'specialpurpose' in normalized_clipping_err:
            print("  Kindle has: 'specialpurpose' (no space/hyphen)")
            if 'special-purpose' in normalized_pdf:
                print("  PDF has: 'special-purpose' (with hyphen)")
...
def test_anchor_based_matching():
    ...
    for anchor_size in [5, 4, 3]:
        ...
        if begin_pos == -1:
            print(f"  ✗ Beginning anchor not found")
            continue
        if end_pos == -1:
            print(f"  ✗ Ending anchor not found")
            continue
        ...
        if extracted == normalized_clipping:
            ...
            print(f"  ✗ Extracted text differs from clipping")
            if len(extracted) != len(normalized_clipping):
                print(f"    Length mismatch: {len(extracted)} vs {len(normalized_clipping)}")
...
def test_position_based_with_errors():
...
def test_position_based_with_flexible_length():
    """
...
def run_all_tests():
    ...
    test_clipping_with_errors()
    
    # Test 3: Anchor-based matching
    results['anchor_based'] = test_anchor_based_matching()
    
    # Test 4: Fuzzy anchor matching
    results['fuzzy_anchor'] = test_fuzzy_anchor_matching()
    
    # Test 5: Position-based extraction
    results['position_based'] = test_position_based_extraction()
    
    # Test 6: Position-based with errors (simple)
    results['position_based_errors_simple'] = test_position_based_with_errors()
    
    # Test 7: Position-based with flexible length (smart)
    results['position_based_flexible'] = test_position_based_with_flexible_length()
