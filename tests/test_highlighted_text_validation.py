...
def extract_highlighted_text_from_pdf(pdf_path: str) -> dict:
    ...
    pdf_name = pdf_path.split('/')[-1].replace('test_highlighted_text_', '').replace('.pdf', '')
    source_pdfs = {
...
def test_highlighted_text_matches_clippings():
    ...
    for dataset in datasets:
        ...
        print(f"\n{'='*80}")
        print(f"🔎 MATCHING EXPECTED vs ACTUAL HIGHLIGHTED TEXT")
        print(f"{'='*80}")
        ...
        for expected in expected_highlights:
            ...
            if page not in actual_highlights:
                ...
                print(f"   Expected: \"{expected_text[:60]}...\"" if len(expected_text) > 60 else f"   Expected: \"{expected_text}\"")
                continue
            ...
            if best_match_ratio >= 0.80:
                ...
                if best_match_ratio < 0.95:
                    print(f"   Expected: \"{expected_text[:60]}...\"" if len(expected_text) > 60 else f"   Expected: \"{expected_text}\"")
                    print(f"   Actual:   \"{best_match_text[:60]}...\"" if len(best_match_text) > 60 else f"   Actual:   \"{best_match_text}\"")
            else:
                unmatched_expected.append(expected)
                print(f"\n❌ Page {page}: Poor match {best_match_ratio*100:.1f}%")
                print(f"   Expected: \"{expected_text[:60]}...\"" if len(expected_text) > 60 else f"   Expected: \"{expected_text}\"")
                print(f"   Actual:   \"{best_match_text[:60]}...\"" if best_match_text and len(best_match_text) > 60 else f"   Actual:   \"{best_match_text}\"")
        
        # Results
        match_percentage = (matched_count / len(expected_highlights) * 100) if expected_highlights else 0
        ...
        if unmatched_expected:
            ...
            for h in unmatched_expected[:5]:  # Show first 5
                text = h['normalized']
                print(f"      Page {h['page']}: \"{text[:60]}...\"" if len(text) > 60 else f"      Page {h['page']}: \"{text}\"")
